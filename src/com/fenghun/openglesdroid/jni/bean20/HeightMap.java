...
public class HeightMap {
	...
	public static final int SIZE_PER_SIDE = 32;	// 每个面有32个单位
	public static final float MIN_POSITION = -5f;
	public static final float POSITION_RANGE = 10f;

	public final int[] vbo = new int[1];
	public final int[] ibo = new int[1];
	...
	public HeightMap(ErrorHandler errorHandler) {
		try {
			...
			for (int y = 0; y < yLength; y++) {
				for (int x = 0; x < xLength; x++) {
					...
					// Build our heightmap from the top down, so that our triangles are counter-clockwise.逆时针
			...
			if (vbo[0] > 0 && ibo[0] > 0) {
				GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, vbo[0]);	// 绑定vertex buffer
				...
				GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, ibo[0]);	// 绑定index buffer
				...
				GLES20.glBindBuffer(GLES20.GL_ARRAY_BUFFER, 0);	// 解绑vertex buffer
				GLES20.glBindBuffer(GLES20.GL_ELEMENT_ARRAY_BUFFER, 0);// 解绑index buffer
			} else {
	...
	public void render(int positionAttribute,int normalAttribute,int colorAttribute) {
		if (vbo[0] > 0 && ibo[0] > 0) {				
